...
function main() {
    ...
    // Função principal para desenhar o bolo de chocolate -> foi a unica parte que devevemos mudar para desenhar os obstaculos
    ...
    function drawChocolateCake(x_pos, z_pos) {
        ...
        // Ajuste de altura base (chão) - ajustado conforme seu código do Chococat
        let AtualY = -1.3;

        // a ordem ta de baixo pra cima -> base + recheio + massa + recheio + massa + cobertura + enfeite

        //  Massa (Base)
        drawCube(1.0, massaColor[0], massaColor[1], massaColor[2], x_pos, AtualY, z_pos, width, 0.3, depth);
        AtualY += 0.15 + 0.05; // Sobe metade da altura da massa anterior + metade da próxima

        // Recheio 1
        drawCube(1.0, recheioColor[0], recheioColor[1], recheioColor[2], x_pos, AtualY, z_pos, width, 0.1, depth);
        AtualY += 0.05 + 0.15;
        ...
        // Cobertura (Ganache)
        drawCube(1.0, coberturaColor[0], coberturaColor[1], coberturaColor[2], x_pos, AtualY, z_pos, width + 0.05, 0.15, depth + 0.05);


        // Pequenos cubos coloridos no topo
        drawCube(0.05, 1.0, 1.0, 1.0, x_pos - 0.5, AtualY + 0.1, z_pos - 0.2, 1, 1, 1);
